...
function setupDelegation() {
  ...
  document.addEventListener("input", e => {
    ...
    if (t.matches("[data-ing-name]"))        { currentProduct().ingredients[Number(t.dataset.ingName)].name=t.value; return; }
    ...
    if (t.matches("[data-master-field]"))    { const f=t.dataset.masterField; if(["directCost","price","directPackaging","directShipping","directOther"].includes(f)) refreshCostKpis(); return; }
  });
  ...
  document.addEventListener("change", e => {
    ...
    if (t.matches("[data-ing-name]"))          { currentProduct().ingredients[Number(t.dataset.ingName)].name=t.value; scheduleRender(); return; }
